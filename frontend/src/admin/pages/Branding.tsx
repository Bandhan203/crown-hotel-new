  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h2 className="text-slate-800 font-semibold text-base mb-3 flex items-center gap-2">
          <MdColorLens style={{ color: accentColor }} />
          Live Sidebar Preview
        </h2>
        <div className="flex items-center gap-4 bg-gray-50 border border-gray-200 rounded-lg px-5 py-4">
          <div className="w-2.5 h-8 rounded-full" style={{ backgroundColor: accentColor }} />
          <div>
            <p className="text-lg font-bold text-slate-800" style={{ fontFamily: '"Gilda Display", serif' }}>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">These values appear in the admin sidebar, login screen, and public site header.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-gray-600 mb-1">Hotel / Site Name <span className="text-red-400">*</span></label>
            <input
              value={draft['site_name'] || ''}
              onChange={e => setDraft(p => ({ ...p, site_name: e.target.value }))}
              placeholder="Hotel Crown"
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-slate-800 text-sm outline-none focus:border-opacity-100 transition"
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Admin Panel Tagline</label>
            <input
              value={draft['admin_tagline'] || ''}
              onChange={e => setDraft(p => ({ ...p, admin_tagline: e.target.value }))}
              placeholder="Hotel Management System"
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-slate-800 text-sm outline-none transition"
              onFocus={e => (e.currentTarget.style.borderColor = accentColor)}
              onBlur={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)')}
            />
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Public Site Tagline</label>
            <input
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Footer Copyright</label>
            <input
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          {PRESET_COLORS.map(preset => (
            ...
            >
              {draft['admin_accent_color'] === preset.value && (
                <span className="absolute inset-0 flex items-center justify-center text-slate-800 text-xs font-bold">✓</span>
              )}
              <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] text-gray-500 whitespace-nowrap opacity-0 group-hover:opacity-100 transition">
                {preset.name}
              </span>
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 mt-8">
          <div
            className="w-11 h-11 rounded-xl border-2 border-gray-300 cursor-pointer overflow-hidden shrink-0 shadow-lg"
            style={{ backgroundColor: accentColor }}
          ...
          <div className="flex-1">
            ...
            <input
              ...
              onChange={e => setDraft(p => ({ ...p, admin_accent_color: e.target.value }))}
              placeholder="#aa8453"
              className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-slate-800 font-mono text-sm outline-none transition"
              onFocus={e => (e.currentTarget.style.borderColor = accentColor)}
              onBlur={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)')}
            />
          </div>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 hover:bg-white/10 text-gray-600 rounded-lg text-xs transition mt-4"
        ...
        <div className="mt-4 rounded-lg overflow-hidden h-8" style={{ background: `linear-gradient(135deg, ${accentColor}, ${accentColor}99)` }}>
          <div className="h-full flex items-center justify-center text-slate-800 text-xs font-medium opacity-80">
            {accentColor} — Preview
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">Image URLs used in the public-facing website.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-gray-600 mb-1">Admin Logo URL</label>
            <input
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">About Section Image URL</label>
            <input
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Primary CTA Label</label>
            <input
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Primary CTA Link</label>
            <input
