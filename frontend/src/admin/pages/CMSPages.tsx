    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="px-4 py-2 bg-[#1a1a1a] border border-white/10 rounded-lg text-white text-sm focus:border-primary outline-none"
          >
      ...
      {loading || !page ? (
        ...
        <div className="space-y-5">
          <div className="bg-[#1a1a1a] border border-white/10 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1">Page Subtitle</label>
              <input value={page.subtitle} onChange={(e) => updateField('subtitle', e.target.value)}
                className="w-full px-3 py-2 bg-[#0f0f0f] border border-white/10 rounded-lg text-white text-sm focus:border-primary outline-none" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm text-gray-300 mb-1">SEO Meta Description</label>
          ...
          <div className="bg-[#1a1a1a] border border-white/10 rounded-xl p-5">
            <h2 className="text-white font-semibold text-lg mb-4">Dynamic Section Blocks</h2>
            <div className="space-y-3 mb-5">
              {Object.entries(page.extra_content || {}).map(([key, val]) => (
                <div key={key} className="flex items-start gap-3 bg-[#0f0f0f] p-3 rounded-lg border border-white/5">
                  <div className="flex-1">
                    ...
                    <textarea value={val} onChange={(e) => setPage({ ...page, extra_content: { ...page.extra_content, [key]: e.target.value } })}
                      className="w-full bg-transparent text-white text-sm outline-none resize-none" rows={2} />
                  </div>
            ...
            <div className="flex gap-2 items-start mt-4 pt-4 border-t border-white/10">
              <input value={extraKey} onChange={(e) => setExtraKey(e.target.value)} placeholder="Block Key (e.g., home_video_url)"
                className="w-1/3 px-3 py-2 bg-[#0f0f0f] border border-white/10 rounded-lg text-white text-sm focus:border-primary outline-none" />
              <input value={extraValue} onChange={(e) => setExtraValue(e.target.value)} placeholder="Block Content"
                className="flex-1 px-3 py-2 bg-[#0f0f0f] border border-white/10 rounded-lg text-white text-sm focus:border-primary outline-none" />
